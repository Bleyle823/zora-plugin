...
async function getZoraActions({ getClients }) {
  return [createCoinAction(getClients), tradeCoinAction(getClients)];
}
async function ensureMetadataUri(runtime, params) {
  ...
  const image = params.image?.trim() || (defaultImageCid ? `ipfs://${defaultImageCid}` : void 0);
...
function createCoinAction(getClients) {
  return {
    name: "CREATE_COIN",
    ...
    handler: async (runtime, message, state, _options, callback) => {
      try {
        ...
        const coinParams = {
          ...
          symbol: parameters.symbol,
          uri: resolvedUri,
          payoutRecipient: parameters.payoutRecipient,
          platformReferrer: parameters.platformReferrer || void 0,
    ...
    examples: [
      [
        {
          user: "{{user1}}",
          content: { text: "Create a coin named 'My Awesome Coin' with symbol 'MAC'" }
        },
        {
          user: "{{user2}}",
          content: { text: "I'll create that coin for you", action: "CREATE_COIN" }
        }
      ],
      [
        {
          user: "{{user1}}",
          content: { text: "Deploy a new creator coin with metadata URI and payout address" }
        },
        {
          user: "{{user2}}",
          content: { text: "Creating your creator coin now", action: "CREATE_COIN" }
        }
      ],
      [
        {
          user: "{{user1}}",
          content: { text: "Launch a new token on Zora platform" }
        },
        {
          user: "{{user2}}",
          content: { text: "Launching your token on Zora", action: "CREATE_COIN" }
        }
      ]
    ]
  };
}
function tradeCoinAction(getClients) {
  return {
    name: "TRADE_COIN",
    ...
    handler: async (runtime, message, state, _options, callback) => {
      try {
        ...
        const tradeParameters = {
          ...
          buy: {
            type: "erc20",
            address: parameters.coinAddress
          },
    ...
    examples: [
      [
        {
          user: "{{user1}}",
          content: { text: "Buy 0.001 ETH worth of coin at address 0x4e93a01c90f812284f71291a8d1415a904957156" }
        },
        {
          user: "{{user2}}",
          content: { text: "I'll execute that trade for you", action: "TRADE_COIN" }
        }
      ],
      [
        {
          user: "{{user1}}",
          content: { text: "Trade ETH for a creator coin with 5% slippage tolerance" }
        },
        {
          user: "{{user2}}",
          content: { text: "Executing the trade with 5% slippage", action: "TRADE_COIN" }
        }
      ],
      [
        {
          user: "{{user1}}",
          content: { text: "Swap ETH for a specific coin token" }
        },
        {
          user: "{{user2}}",
          content: { text: "Swapping ETH for your coin", action: "TRADE_COIN" }
        }
      ]
    ]
  };
}
function composeParameterContext(actionName, state, description) {
...
async function generateParameters(runtime, context, schema) {
...
function composeResponseContext(actionName, result, state) {
